...
from craigslistbargain.neural.symbols import markers
...
class Batch(object):
    ...
    def convert_device(self, device):
        # 遍历这个batch里面登记过的tensor字段，如果设备不太，就迁移到新设备上
    ...
    def int_to_onehot(tensor, onehot_size):
        ...
        onehot = torch.zeros((real_size, onehot_size), device=tensor.device).scatter(1, tensor, 1)   # 用scatter在指定位置填1
        return onehot.reshape(batch_size, -1)   # reshape成batch形式
    ...
    def reshape_tensors(self, attributes):
        ...
        for v in attributes:
            ...
            d = len(tmp.shape)
            if d == 1:
                tmp = tmp.reshape(1, -1)   # 1维tensor补成2维
            if d > 2:   # 3维以上压成2维
    ...
    def convert_data(cls, encoder_args, decoder_args, context_data, vocab, cuda=False):
        # 把原始的 encoder_args / decoder_args / context_data 变成模型输入
        ...
        only_run = False
        if decoder_args is None:   # 如果没有decoder_args，当前就只做推理，不做监督训练
            only_run = True
        ...
        # 后面 RawBatch.act_0 ~ act_3 就是从这里来
        # else:
        ...
        # 这里的uttr是一个list，里面每个元素都是一条样本的token tensor
        ...
        target_prob = cls.to_variable(target_prob, 'float', cuda)   # 训练标签
        ...
        """
            非常重要，构建三个state
        """
        state_sentence = torch.cat([encoder_intent, encoder_price], dim=-1)   # state_sentence，[intent_onehot, price]；用one-hot动作历史，再拼原始价格轨迹
        state_extra = encoder_extra
        # 就是extra，后面_create_one_batch写了，extra = [role_onehot + [turn_ratio] + last_two_prices]
        state_obs = torch.cat([encoder_intent, encoder_price.mul(encoder_pmask), encoder_pmask], dim=-1)
        # state_obs [intent_onehot, masked_price, pmask]；这里的价格是：有价格时保留；没价格时强制变0；旁边再单独拼一个pmask
        uttr = encoder_tokens

        return (state_sentence, state_extra, state_obs), uttr, \
               (target_intent, target_price, target_pact, target_pmask), target_prob
        # 返回四种东西
        # state三元组、uttr、act四元组、prob
