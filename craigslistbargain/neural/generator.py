...
from craigslistbargain.neural.symbols import markers, category_markers, sequence_markers
...
class LFSampler(Sampler):
    ...
    def _is_switch_aware(self):
        return getattr(self.model, '__class__', None).__name__ == 'SwitchAwareHistoryModel'
    ...
    def _run_batch_tom_identity(
        ...
        if self._is_switch_aware():
            if only_identity:
                raise NotImplementedError(
                    "SwitchAwareHistoryModel online path does not support only_identity=True."
                )
        ...
        if only_identity:
            identity, next_hidden = self.model.encoder.identity(
                batch.identity_state,
                batch.extra,
                hidden_state,
                uttr=batch.uttr,
    ...
    def generate_batch(self, batch, gt_prefix=1, enc_state=None, whole_policy=False, special_actions=None,
                   temperature=1, acpt_range=None, hidden_state=None, force_switch_prob=None):
        ...
        if self.model_type in ['sl', 'rl']:
            ...
            # tom model
            out, next_hidden, _ = self._run_batch_tom_identity(
                batch,
                hidden_state,
                force_switch_prob=force_switch_prob,
            )
        ...
        if self.model_type == 'rl':
            # 旧 RL 主模型：4-way pact action
            p_act, p_policy = self.softmax_with_mask(p_policy)
            price = int(p_act.item())
            prob = prob * p_policy[0, price].item()
            original_price = price

        else:
            # 非 RL：既兼容旧标量 price，也兼容新的 100-bin price logits
            if torch.is_tensor(p_policy) and p_policy.numel() > 1:
                if p_policy.dim() == 1:
                    p_policy = p_policy.view(1, -1)

                price_bin, p_policy = self.softmax_with_mask(p_policy)
                price_bin = int(price_bin.item())

                # 100-bin -> [0, 1] normalized price
                denom = max(p_policy.size(-1) - 1, 1)
                price = float(price_bin) / float(denom)

                prob = prob * p_policy[0, price_bin].item()
                original_price = price_bin
            else:
                # 旧标量 price head
                price = float(p_policy.item())
                original_price = price
        ...
        # 只有 price-action intent 才保留价格
        if intent_idx not in self.price_actions:
            price = None
        ...
        # SwitchAware: 把 aux outputs 也带出去，供 tom_inference / 打印使用
        if self._is_switch_aware():
