...
class PriceScaler(object):
    ...
    def unscale_price(cls, kb, price):
        p_in = PriceTracker.get_price(price)
        ...
        #     "w=", w,
        #     "c=", c,
        #     "real_before_round=", p_real,
        #     "real_after_round=", p_round,
        # )

        if isinstance(price, Entity):
            return price._replace(canonical=price.canonical._replace(value=p_round))
        elif isinstance(price, CanonicalEntity):
            return price._replace(value=p_round)
        else:
            return p_round
